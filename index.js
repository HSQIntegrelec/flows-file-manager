...
function normalizeString(stringElement) {
  let normalized = stringElement.replace(/[^a-zA-Z0-9\.]/g, '-').replace(/----/g, '-').replace(/---/g, '-').replace(/--/g, '-').replace(/--/g, '-').toLowerCase();
  if (normalized[0] === '-') {
...
function disambiguate(flowSet, child, attribute) {
  ...
  flowSet[child].forEach((subelement) => {
    let normalizedAttribute = normalizeString(subelement.config[attribute] || subelement[attribute] || ((subelement.config.site === undefined) ? undefined : subelement.config.site.name) || subelement.type || subelement.id);
...
function constructTreeObjectFromFlowSet(flowSet) {
  ...
  flowSet.subflows.forEach((subflow) => {
    contents.push({
      ...
      folder: 'subflows',
      content: [subflow.export()].concat(subflow.exportContents().sort(compare))
    });
  })
...
function constructTreeFilesFromFlowSet(flowSet, config, rootProjectPath = '.') {
  ...
  if (!config.fileFormat || !config.destinationFolder || !Object.keys(config).includes('tabsOrder') || !config.monolithFilename) {
    console.log(`Erroneous config file, missing key element in the JSON : ${config}`);
    return
  }
  if (!(/^json|yaml|yml$/.test(config.fileFormat.toLowerCase()))) {
    console.log(`Unexpected file format : ' ${config.fileFormat}'. Allowed formats are JSON and YAML.`);
    return
  }
  ...
  if (createSrcFolders(config, rootProjectPath)) {
    ...
    tree.forEach(element => {
      ...
      try {
        fs.writeFileSync(path.join(rootProjectPath, config.destinationFolder, `./${element.folder}/`, `${element.fileName}.${config.fileFormat}`), data);
      } catch (error) {
        console.log(`Could not create source files for element '${element.fileName}' : ${error}`)
        return
      }
    });
  }
 ...
 * @returns 
 */
function constructMonolithObjectFromFlowSet(flowSet, config, overwriteTabsOrder = false) {
  if (!config.fileFormat || !config.destinationFolder || !Object.keys(config).includes('tabsOrder') || !config.monolithFilename) {
    console.log(`Erroneous config file, missing key element in the JSON : ${config}`);
    return
  }
 ...
 * @returns {object} config FlowsFileManager specific config object
*/
function constructMonolithFileFromFlowSet(flowSet, config, rootProjectPath = '.', overwriteTabsOrder = false) {
  const flowConfig = constructMonolithObjectFromFlowSet(flowSet, config, overwriteTabsOrder);
...
function constructFlowSetFromTreeFiles(config, rootProjectPath) {
  ...
  if (!(/^json|yaml|yml$/.test(config.fileFormat.toLowerCase()))) {
    console.log(`Unexpected file format (it should be JSON or YAML exclusively) : ${config.fileFormat}`);
    return
  }
  if (!fs.existsSync(path.join(rootProjectPath || '.', config.destinationFolder))) {
    console.log(`Disparity in config.destinationFolder '${config.destinationFolder}' and existing files.`);
    return
  }
  ...
  ['tabs', 'subflows', 'config-nodes'].forEach((nodeType) => {
    fs.readdirSync(path.join(rootProjectPath || '.', config.destinationFolder, nodeType)).forEach((filename) => {
      ...
      let treeFilePath = path.join(rootProjectPath || '.', config.destinationFolder, nodeType, filename);
